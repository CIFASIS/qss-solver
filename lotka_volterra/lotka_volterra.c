...
MOD_definition (int i, double *x, double *d, double *alg, double t, double *dx)
{
    switch (i)
    {
        case 0:
            ...
            dx[2] = (-(1.000000000000000055511151e-01) * x[4] * x[1] - (1.000000000000000055511151e-01) * x[5] * x[0]
                    + (1.000000000000000055511151e-01) * x[1]) / 2;
            dx[3] = ((1.000000000000000055511151e-01) * x[2] * 2 - (1.000000000000000055511151e-01) * x[0] * x[6] * 2
                    - (2.000000000000000111022302e-01) * x[1] * x[5] - (1.000000000000000055511151e-01) * x[2] * 2 * x[4]) / 6;
            return;
        case 1:
            dx[1] = 1.000000000000000055511151e-01 * x[0] * x[4] - 1.000000000000000055511151e-01 * x[4];
            dx[2] = (-(1.000000000000000055511151e-01) * x[5] + (1.000000000000000055511151e-01) * x[5] * x[0]
                    + (1.000000000000000055511151e-01) * x[1] * x[4]) / 2;
            dx[3] = ((1.000000000000000055511151e-01) * x[6] * 2 * x[0] + (2.000000000000000111022302e-01) * x[1] * x[5]
                    + (1.000000000000000055511151e-01) * x[4] * x[2] * 2 - (1.000000000000000055511151e-01) * x[6] * 2) / 6;
            return;
    }
}
...
MOD_dependencies (int i, double *x, double *d, double *alg, double t, double *der, int *map)
{
    switch (i)
    {
        case 0:
            ...
            der[0 + 2] = (-(1.000000000000000055511151e-01) * x[4] * x[1] - (1.000000000000000055511151e-01) * x[5] * x[0]
                    + (1.000000000000000055511151e-01) * x[1]) / 2;
            der[0 + 3] = ((1.000000000000000055511151e-01) * x[2] * 2 - (1.000000000000000055511151e-01) * x[0] * x[6] * 2
                    - (2.000000000000000111022302e-01) * x[1] * x[5] - (1.000000000000000055511151e-01) * x[2] * 2 * x[4]) / 6;
            der[4 + 1] = 1.000000000000000055511151e-01 * x[0] * x[4] - 1.000000000000000055511151e-01 * x[4];
            der[4 + 2] = (-(1.000000000000000055511151e-01) * x[5] + (1.000000000000000055511151e-01) * x[5] * x[0]
                    + (1.000000000000000055511151e-01) * x[1] * x[4]) / 2;
            der[4 + 3] = ((1.000000000000000055511151e-01) * x[6] * 2 * x[0] + (2.000000000000000111022302e-01) * x[1] * x[5]
                    + (1.000000000000000055511151e-01) * x[4] * x[2] * 2 - (1.000000000000000055511151e-01) * x[6] * 2) / 6;
            return;
        case 1:
            der[0 + 1] = 1.000000000000000055511151e-01 * x[0] - 1.000000000000000055511151e-01 * x[0] * x[4];
