...
MMO_Generator_::MMO_Generator_ (MMO_StoredDefinition std, MMO_CompileFlags flags) :
        _std (std), _model (NULL), _function (NULL), _package (NULL), _flags (flags), _solver (NULL), _files (NULL), _writer (NULL), _includes (), _fheader ()
{
    if (_flags->output ())
    {
...
MMO_Generator_::_generateModel ()
{
    ...
    if (!_writer->isEmpty (WR_OUTPUT_SIMPLE) || !_writer->isEmpty (WR_OUTPUT_GENERIC))
    {
...
MMO_Generator_::_header ()
{
    ...
    if (_model->imps ())
    {
        ...
        {
            buffer << "#include \"" << Util::getInstance ()->packageName (i) << ".h\"" << endl;
        }
    }
...
MMO_Generator_::_variablesInitCode ()
{
    ...
    {
        Index idx = vi->index ();
        if (!vi->isConstant () && !vi->isParameter () && (vi->hasAssignment () || vi->hasStartModifier () || vi->hasEachModifier ()))
        {
            buffer << _model->printInitialAssignment (vi, indent);
            _writer->write (&buffer, WR_START_CODE);
        }
