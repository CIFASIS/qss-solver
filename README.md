...
automatically generates the plain C code that contains an appropiate 
model definition needed by the 
[QSS Solver engine](https://github.com/CIFASIS/qss-solver/tree/qss-solver-dev/src/engine)
together with all the configuration files (makefiles, config, etc.).
The compiler documentation can be found [here](http://www.cifasis-conicet.gov.ar/jfernandez/mmoc/)
