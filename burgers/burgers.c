...
MOD_definition (int i, double *x, double *d, double *alg, double t, double *dx)
{
    ...
    switch (i)
    {
        ...
        default:
            ...
            if (j >= 1 && j <= 998)
            {
                dx[1] = d[(j)] * (x[(j) * 3] + x[(j - 1) * 3]) / 2.0 * (-x[(j) * 3] + x[(j - 1) * 3]) * 1000
                        + (1.0 - d[(j)]) * (x[(j + 1) * 3] + x[(j) * 3]) * (-x[(j + 1) * 3] + x[(j) * 3]) * 1000;
            }
    }
}
...
MOD_dependencies (int i, double *x, double *d, double *alg, double t, double *der, int *map)
{
    ...
    switch (i)
    {
        case 0:
            ...
            der[2997 + 1] = d[999] * (x[2997] + x[2994]) / 2.0 * (-x[2997] + x[2994]) * 1000
                    + (1.0 - d[999]) * (x[0] + x[2997]) * (-x[0] + x[2997]) * 1000;
            break;
        case 1:
            der[0 + 1] = d[0] * (x[0] + x[2997]) / 2.0 * (-x[0] + x[2997]) * 1000 + (1.0 - d[0]) * (x[3] + x[0]) / 2.0 * (-x[3] + x[0]) * 1000;
            break;
    ...
    if (j >= 1 && j <= 998)
    {
        der[(j) * 3 + 1] = d[(j)] * (x[(j) * 3] + x[(j - 1) * 3]) / 2.0 * (-x[(j) * 3] + x[(j - 1) * 3]) * 1000
                + (1.0 - d[(j)]) * (x[(j + 1) * 3] + x[(j) * 3]) * (-x[(j + 1) * 3] + x[(j) * 3]) * 1000;
    }
    j = i;
    if (j >= 1 && j <= 998)
