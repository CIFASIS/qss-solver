...
MOD_definition (int i, double *x, double *d, double *alg, double t, double *dx)
{
    switch (i)
    {
        case 0:
            ...
            return;
        case 1:
            dx[1] = -9.800000000000000710542736e+00 - 1.000000000000000055511151e-01 * x[4]
                    - d[(1)] * ((x[0] - d[(0)]) * 1.000000000000000000000000e+06 + x[4] * 30.0);
...
MOD_dependencies (int i, double *x, double *d, double *alg, double t, double *der, int *map)
{
    ...
    {
        case 0:
            der[4 + 1] = -9.800000000000000710542736e+00 - 1.000000000000000055511151e-01 * x[4]
                    - d[(1)] * ((x[0] - d[(0)]) * 1.000000000000000000000000e+06 + x[4] * 30.0);
            der[4 + 2] = (-d[(1)] * ((1.000000000000000000000000e+06) * x[1] + 30.0 * x[5]) - (1.000000000000000055511151e-01) * x[5]) / 2;
            der[4 + 3] = (-d[(1)] * ((1.000000000000000000000000e+06) * x[2] * 2 + 30.0 * x[6] * 2) - (1.000000000000000055511151e-01) * x[6] * 2)
                    / 6;
            return;
        case 1:
            der[0 + 1] = x[4];
