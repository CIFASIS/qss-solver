...
MMOWriter::MMOWriter (string file) :
        _indent (1), _modelName (), _out (file.c_str (), ios::out | ios::in | ios::trunc),
...
MMOWriter::~MMOWriter ()
{
    _out << "\tannotation(\n\t experiment(\n\t\t MMO_Description=\"" << _modelName << " automatic model translation.\"";
    _out << ",\n\t\t MMO_Solver=LIQSS2,\n\t\t StartTime=0,\n\t\t StopTime=1,\n\t\t Tolerance= 1e-3,\n\t\t AbsTolerance= 1e-6,\n\t\t";
...
MMOWriter::visit (MMODecl *x)
{
    ...
    if (x->isDerivative ())
    {
        ...
        _out << utils->indent (_indent) << "der(" << x->getId () << ") = " << x->getExp () << ";" << endl;
    }
    if (x->isImplicit ())
    {
    ...
    {
        _out << utils->indent (_indent) << x->getId () << " = " << x->getExp () << ";" << endl;
    }
    ...
    if (x->isReinit ())
    {
        _out << utils->indent (_indent) << "reinit(" << x->getId () << "," << x->getExp () << ");" << endl;
    }
    if (x->isZeroCrossing ())
    {
    ...
    if (x->isState ())
    {
        if (x->hasValue ())
        {
            _out << utils->indent (_indent) << "Real " << x->getId () << "(start = " << x->getValue () << ");" << endl;
        }
        else
        {
    ...
    if (x->isParameter ())
    {
        if (x->hasValue ())
        {
            _out << utils->indent (_indent) << "parameter Real " << x->getId () << " = " << x->getValue () << ";" << endl;
        }
        else
        {
            _out << utils->indent (_indent) << "parameter Real " << x->getId () << ";" << endl;
        }
    }
    if (x->isConstant ())
    {
        if (x->hasValue ())
        {
            _out << utils->indent (_indent) << "constant Integer " << x->getId () << " = " << x->getValue () << ";" << endl;
        }
    }
    if (x->isDiscrete ())
    {
        ...
        if (x->hasValue ())
        {
            _out << utils->indent (_indent) << "discrete Real " << x->getId () << "(start = " << x->getValue () << ");" << endl;
        }
        else
        {
            _out << utils->indent (_indent) << "discrete Real " << x->getId () << ";" << endl;
        }
    }
    if (x->isFunctionInput ())
    {
        _out << utils->indent (_indent) << "input Real " << x->getId () << ";" << endl;
    }
    if (x->isFunctionOutput ())
    {
        _out << utils->indent (_indent) << "output Real " << x->getId () << ";" << endl;
