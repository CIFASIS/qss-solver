        ...
        _bioricaLanguage (false)
{
    ...
    _predefinedFunctions.insert (pair<string, map<string, string> > ("math", mathFunctions));
    _varnum = 0;
}
;
...
MMOUtils::checkPredefinedFunctions (ASTNode* node)
{
    if (node->isFunction ())
    {
        ...
        for (it = _predefinedFunctions.begin (); it != _predefinedFunctions.end (); it++)
        {
...
{
    if (type == AST_RELATIONAL_EQ || type == AST_RELATIONAL_GEQ || type == AST_RELATIONAL_GT || type == AST_RELATIONAL_LEQ
            || type == AST_RELATIONAL_LT || type == AST_RELATIONAL_NEQ || type == AST_PLUS || type == AST_TIMES || type == AST_MINUS
