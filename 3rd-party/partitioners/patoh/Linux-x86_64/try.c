...
void PrintInfo(char *pname, int _k, int *partweights, int cut, int _nconst,
    float *targetweights)
{
  double *tot, *maxi;
  int i, j;
  ...
  for(i = 0; i < _k; ++i)
  {
    ...
    for(j = 0; j < _nconst; ++j)
    {
      double im = (double) ((double) partweights[i * _nconst + j]
          - tot[j] * targetweights[i * _nconst + j])
          / (tot[j] * targetweights[i * _nconst + j]);

      maxi[j] = umax(maxi[j], im);
      printf("%10d (TrgtR:%.3f  imbal:%6.3lf%%) ", partweights[i * _nconst + j],
          targetweights[i * _nconst + j], 100.0 * im);
    }
  }
...
int main(int argc, char *argv[])
{
  ...
  if(PT == 'T')
  {
    ...
    printf("\n");
    PrintInfo("Single Constraint Skewed", args._k, partweights, cut, 1,
        skewedtweights);
  }
  ...
  if(PT == 'T')
  {
    ...
    PaToH_Part(&args, _c, _n, 1, 1, cwghts, nwghts,
        xpins, pins, skewedtweights, partvec, partweights, &cut);
    PrintInfo("With FixCells Skewed", args._k, partweights, cut, 1,
        skewedtweights);
  }
  ...
  if(PT == 'M' || PT == 'A' || PT == 'T')
  {
    ...
    if(PT == 'T')
    {
      PaToH_Part(&args, _c, _n, _nconst, 0, cw, nwghts,
          xpins, pins, skewedtweights, partvec, partweights, &cut);
      PrintInfo("MultiConstraint Skewed", args._k, partweights, cut, _nconst,
          skewedtweights);
    }
    else
    {
      PaToH_MultiConst_Partition(&args, _c, _n, _nconst, cw, xpins, pins,
          partvec, partweights, &cut);
      PrintInfo("MultiConstraint", args._k, partweights, cut, _nconst,
          targetweights);
    }
    free(cw);
  }
