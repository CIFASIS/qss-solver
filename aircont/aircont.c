...
MOD_definition (int i, double *x, double *d, double *alg, double t, double *dx)
{
    ...
    switch (i)
    {
        ...
        default:
            ...
            if (j >= 0 && j <= 199)
            {
                dx[1] = (__PAR_THA / __PAR_RES[(j)] - __PAR_POT[(j)] * d[(j + 2)] - x[(j) * 4] / __PAR_RES[(j)] + d[(205)] / __PAR_RES[(j)])
                        / __PAR_CAP[(j)];
...
MOD_dependencies (int i, double *x, double *d, double *alg, double t, double *der, int *map)
{
    ...
    if (j >= 0 && j <= 199)
    {
        der[(j) * 4 + 1] = (__PAR_THA / __PAR_RES[(j)] - __PAR_POT[(j)] * d[(j + 2)] - x[(j) * 4] / __PAR_RES[(j)] + d[(205)] / __PAR_RES[(j)])
                / __PAR_CAP[(j)];
