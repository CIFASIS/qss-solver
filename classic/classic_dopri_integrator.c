...
void
DOPRI_solout (long nr, double xold, double x, double* y, unsigned n, int* irtrn, int *jroot)
{
    ...
    if (!is_sampled)
        {
            clcData->totalSteps++;
            CLC_save_step (simOutput, simDataDopri.solution, simDataDopri.solution_time, x, simDataDopri.totalOutputSteps[0], y, clcData->d, clcData->alg);
...
DOPRI_integrate (SIM_simulator simulate)
{
    ...
    setInitRealTime();
#endif
    res = dopri5 (size, DOPRI_model, t, x, _ft, &rel_tol, &abs_tol, 0, DOPRI_solout, 2,
    stdout,
