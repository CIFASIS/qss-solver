...
#define HIST 1e-12
void
DASSL_events (const int *n, const double *t, const double *x, const double *dx, const int *nrt, double *res, const double *rPar, const int *iPar)
{
    int i;
    double out;
    //DASSL_update_x(x);
...
void
DASSL_model (const double *t, const double *x, const double *dx, const double *CJ, double *res, int *iRes, const double *rPar, const int *iPar)
{
    int i;
...
DASSL_integrate (SIM_simulator simulate)
{
    ...
    const int num_steps = (is_sampled ? ceil (_ft / step_size) + 2 : MAX_OUTPUT_POINTS);
    ...
    while (t < _ft)
        {
            ...
            if (tout > _ft)
                tout = _ft;
            ddaskr_ (DASSL_model, &size, &t, x, dx, &tout, info, &rel_tol, &abs_tol, &method_info, rwork, &lrw, iwork, &liw,
            NULL,
            ...
            if (!is_sampled)
            ...
            else
                {
                    if (!event_detected)
                        {
                            if (fabs (tout - solution_time[clcData->totalOutputSteps - 1]) > step_size / 10)
                                {
                                    CLC_save_step (simOutput, solution, solution_time, tout, clcData->totalOutputSteps, x, clcData->d, clcData->alg);
                                    clcData->totalOutputSteps++;
                                }
    ...
    if (simulator->settings->debug == 0 || simulator->settings->debug > 1)
        {
            ...
            SD_print (simulator->simulationLog, "Output steps: %d", clcData->totalOutputSteps);
            SD_print (simulator->simulationLog, "Simulation steps: %d", clcData->totalSteps);
