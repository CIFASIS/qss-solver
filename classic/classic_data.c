...
CLC_Data (int states, int discretes, int events, int inputs, int algebraics, string name)
{
    ...
    p->params = SD_Parameters (settings->derdelta, settings->zchyst, settings->minstep, settings->symdiff, settings->lps, settings->nodesize, settings->pm,
                               settings->dt, settings->dtSynch, settings->partitionerOptions, settings->jacobian);
