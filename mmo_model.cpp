...
MMOModel::MMOModel (string prefix, bool replace, WR_Type type) :
        _replace (replace), _type (type), _imports (), _functions (),
        _variableParameters (), _prefix (), _constantAlgebraics (), _name (), _variables ()
{
    if (_replace)
    {
        _prefix = prefix;
...
MMOModel::MMOModel (string name) :
        _replace (false), _type (WR_MMO), _imports (), _functions (), _variableParameters (),
        _prefix (), _constantAlgebraics (), _name (), _variables ()
{
}
...
{
    for (pair<string, pair<string, list<string> > > def = eq.firstAlgebraic (); !eq.endAlgebraic (); def = eq.nextAlgebraic ())
    {
        MMODecl* exp = new MMODecl (def.first, def.second.first, DEC_ALGEBRAIC_EQUATION);
        _variables.insert (pair<string, list<string> > (def.first, def.second.second));
    ...
    if (eq.hasZeroCrossing ())
    {
        ...
        for (it = eq.firstEvent (); !eq.endEvent (); it = eq.nextEvent ())
        {
            ...
            for (zc_it = zc.firstZC (); !zc.endZC (); zc_it = zc.nextZC ())
            {
                ...
                for (asg_it = it.second.begin (); asg_it != it.second.end (); asg_it++)
                {
                    string asgVar;
                    if (asg_it->first == "")
                    {
                    ...
                    {
                        MMODecl* asg_vars = new MMODecl (asg_it->first, DEC_DISCRETE);
                    ...
                    MMODecl asg = MMODecl (asgVar, asg_exp.getExp (), DEC_ASSIGNMENT, asg_exp.isConditional ());
...
void
MMOModel::_parseEquation (string id, const ASTNode *node, MMODeclType tydec, MMOSectionType type)
{
    ...
    if (eq.hasEquation ())
    {
        ...
        {
            _variables.insert (pair<string, list<string> > (id, eq.getVariables ()));
        }
        _add (exp, type);
...
void
MMOModel::add (string id, bool isDerivative, const ASTNode *value, MMOSectionType type)
{
    MMODeclType t;
    MMODecl *dec = _declarations.findDec (id);
    if (dec == NULL)
...
MMOModel::_reorder ()
{
    ...
    for (expIt = _constantAlgebraics.begin (); expIt != _constantAlgebraics.end (); expIt++)
    {
...
MMOModel::add (const FunctionDefinition &f)
{
    ...
    func->add (adec);
    _functions[fname] = pair<list<string>, ASTNode*> (funcArgs, new ASTNode (*f.getBody ()));
...
{
    for (MMODecl *e = _declarations.first (); !_declarations.end (); e = _declarations.next ())
    {
        if (e->isAlgebraic ())
        {
            if (_equation.findDec (e->getId ()) == NULL)
            {
                stringstream buffer;
                buffer << e->getValue ();
                MMODecl* exp = new MMODecl (e->getId (), buffer.str (), DEC_ALGEBRAIC_EQUATION);
                _add (exp, SEC_EQUATION);
