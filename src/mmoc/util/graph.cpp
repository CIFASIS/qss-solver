...
map<int, set<int>> Graph::graph() { return _graph; }

map<int, set<int>> Graph::hyperGraph() { return _hyperGraph; }

void Graph::addGraphEdge(int orig, int dest)
{
  _graph[orig].insert(dest);
  _graphInputs[dest].insert(orig);
}

void Graph::addHyperGraphEdge(int orig, int dest) { _hyperGraph[orig].insert(dest); }
...
int Graph::graphEdgeWeight(int node, int inf)
{
  int w = 1;
...
int Graph::hyperGraphEdgeWeight(int node)
{
  ...
  for (it = _hyperGraph[node].begin(); it != _hyperGraph[node].end(); ++it) {
    if (*it != node) {
      if ((*it == node + 1) && (_hwmap[node].find(node + 1) != _hwmap[node].end())) {
        w += GRP_Weight(_profile, GRP_VIRT);
      } else {
        w += edgeWeight(node);
      }
...
int Graph::graphEdges() { return _graphEdges; }

int Graph::graphNodeEdges(int node) { return _graph[node].size(); }

int Graph::hyperGraphEdges() { return _hyperGraph.size(); }

bool Graph::empty() { return _graph.empty(); }

int Graph::nodeWeight(int node) { return _graphInputs[node].size() + _graphDiscretes[node] + 1; }
