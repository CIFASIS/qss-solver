...
void MOD_definition(int idx, double *x, double *d, double *a, double t, double *dx)
{
	...
	if (_is_var_x(idx)) {
		...
		if ((i >= 2 && i <= 502)) {
			...
			_der_x(i,1) = (-2*_G*((_x(i-1,1)-_x(i,1))*(_x(i,0)-_x(i-1,0)+_UTH)*_satdx(i-1)+(_x(i-1,0)-_UTH)*_satx(i-1)*_x(i-1,1))-_x(i,1))/2;
		}
		return;
	}
}
...
void MOD_dependencies(int idx, double *x, double *d, double *a, double t, double *dx, int *map)
{
	...
	if (_is_var_x(idx)) {
		...
		if ((i >= 2 && i <= 502)) {
			...
			_eval_dep_x(i,2) = (-2*(_x(i-1,1)*_satx(i-1)*(_x(i-1,0)-_UTH)-(_x(i-1,1)-_x(i,1))*_satdx(i-1)*(_x(i-1,0)-_UTH-_x(i,0)))*_G-_x(i,1))/2;	}
	
		_apply_usage_eq_2(_d1+1);
		if ((i >= 2 && i <= 502)) {
			_eval_dep_x(i,1) = _UOP-_x(i,0)-_G*(_satx(i-1)*pow((_x(i-1,0)-_UTH),2)-_satdx(i-1)*pow((_x(i-1,0)-_x(i,0)-_UTH),2));
			_eval_dep_x(i,2) = (-_x(i,1)-2*_G*(_satdx(i-1)*(_x(i,1)-_x(i-1,1))*(_x(i-1,0)-_UTH-_x(i,0))+(_x(i-1,0)-_UTH)*_x(i-1,1)*_satx(i-1)))/2;	}
