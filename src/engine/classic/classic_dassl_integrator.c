                 ...
                 const int *iPar)
{
  ...
  for (i = 0; i < clcData->states; i++) res[i] -= dx[i];
  clcData->funEvaluations++;
}

void DASSL_integrate(SIM_simulator simulate)
{
  ...
  while (t < _ft) {
    ...
    if (tout > _ft) tout = _ft;
    ddaskr_(DASSL_model, &size, &t, x, dx, &tout, info, &rel_tol, &abs_tol, &method_info, rwork, &lrw, iwork, &liw, NULL, NULL, NULL, NULL,
            DASSL_events, &numofconstr, root_output);
    if (method_info < 0) {
    ...
    if (method_info == 5) {
      ...
      if (is_sampled) event_detected = 1;
      info[0] = 0;
    }
    if (!is_sampled) {
      CLC_save_step(simOutput, solution, solution_time, t, clcData->totalOutputSteps, x, clcData->d, clcData->alg);
