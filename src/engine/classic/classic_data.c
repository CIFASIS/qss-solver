...
void CLC_freeData(CLC_data data)
{
  ...
  free(data->nDS);
  for (i = 0; i < states; i++) {
    if (data->SD[i] != NULL) free(data->SD[i]);
    if (data->DS[i] != NULL) free(data->DS[i]);
  }
  free(data->SD);
  free(data->DS);
  if (data->inputs > 0) {
...
CLC_model CLC_Model(CLC_eq f, CLC_zc zeroCrossing, CLC_hnd handlerPos, CLC_hnd handlerNeg, CLC_jac jac)
{
