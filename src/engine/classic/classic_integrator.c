...
void CLC_initialize(SIM_simulator simulate)
{
  ...
  for (i = 0; i < forUL; i++) {
    ...
    clcModel->events->zeroCrossing(i, clcData->x, clcData->d, clcData->alg, t, zc);
    s = sign(zc[0]);
...
#ifdef DEBUG
      printf("Re-checking handler. Evaluating zc %d at %g res=%g. Sign = %d\n", j, t, out, clcData->event[j].zcSign);
#endif
      if (out * clcData->event[j].zcSign < 0) {
#ifdef DEBUG
        printf("Must execute linked handler %d at %g. out=%g sign=%d dir=%d\n", j, t, out, clcData->event[j].zcSign,
               clcData->event[j].direction);
#endif
        if (/*out>HIST &&*/ clcData->event[j].zcSign < 0 && clcData->event[j].direction >= 0 && clcData->fired[j] < 3) {
#ifdef DEBUG
          printf("Execution handler pos for zc %d at %g\n", j, t);
#endif
          ...
          // reset_x(x);
          clcData->totalEvents++;
        } else if (/*out<-HIST &&*/ clcData->event[j].zcSign > 0 && clcData->event[j].direction <= 0 && clcData->fired[j] < 3) {
#ifdef DEBUG
          printf("Execution handler neg for zc %d at %g\n", j, t);
#endif
...
#ifdef DEBUG
        printf("Changing sign of %d to %d at %g\n", j, clcData->event[j].zcSign, t);
#endif
      }
    }
  }
  if (iter >= 100) {
    printf("Maximum number of event iterations. Probably there is a loop dependency in the events\n");
  }
  if (iwork != NULL) {
