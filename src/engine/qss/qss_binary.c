...
#ifdef QSS_PARALLEL
void BT_PAR_initTree(BT_tree tree)
#else
void BT_initTree(BT_tree tree)
#endif
{
  if (tree->size == 1) {
#ifdef QSS_PARALLEL
...
#ifdef QSS_PARALLEL
void BT_PAR_updateTree(SC_scheduler scheduler, BT_tree tree, int *inf, int cant, int idx)
#else
void BT_updateTree(SC_scheduler scheduler, BT_tree tree, int *inf, int cant, int idx)
#endif
{
  if (tree->size == 1) {
    return;
...
#ifdef QSS_PARALLEL
void BT_PAR_updateC(SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
#else
void BT_updateC(SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
#endif
{
  ...
  if (simTime->type == ST_State) {
    ...
    BT_UPDATE_TREE(scheduler, scheduler->state->states, simData->SD[stateVar], simData->nSD[stateVar], update);
    minIdx = scheduler->state->states->nodes[scheduler->state->states->height][0].value;
    minVal = simTime->nextStateTime[minIdx];
  } else if (simTime->type == ST_Event) {
    if (simData->nHD[stateVar]) {
      BT_UPDATE_TREE(scheduler, scheduler->state->states, simData->HD[stateVar], simData->nHD[stateVar], UPDATE_DEPS);
    }
    if (simData->event[stateVar].nLHSSt) {
      ...
      for (i = 0; i < nLHSSt; i++) {
        ...
        if (lp->qMap[j] > NOT_ASSIGNED) {
          BT_UPDATE_TREE(scheduler, scheduler->state->states, simData->SD[j], simData->nSD[j], j);
        }
      }
    }
  }
#else
  BT_UPDATE_TREE(scheduler, scheduler->state->states, simData->SD[stateVar], simData->nSD[stateVar], stateVar);
  minIdx = scheduler->state->states->nodes[scheduler->state->states->height][0].value;
  minVal = simTime->nextStateTime[minIdx];
#endif
...
#ifdef QSS_PARALLEL
void BT_PAR_updateH(SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
#else
void BT_updateH(SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
#endif
{
  ...
  if (simTime->type == ST_State) {
    ...
    if (nStates > 0) {
      ...
      scheduler->state->stepType = ST_State;
#endif
      BT_UPDATE_TREE(scheduler, states, simData->SD[stateVar], simData->nSD[stateVar], update);
...
#endif
      BT_UPDATE_TREE(scheduler, events, simData->SZ[stateVar], simData->nSZ[stateVar], UPDATE_DEPS);
    }
    ...
    if (nStates > 0) {
      if (simData->nHD[stateVar]) {
        ...
        scheduler->state->stepType = ST_State;
#endif
        BT_UPDATE_TREE(scheduler, states, simData->HD[stateVar], simData->nHD[stateVar], UPDATE_DEPS);
      }
      if (simData->event[stateVar].nLHSSt) {
        int i, j;
        ...
        for (i = 0; i < nLHSSt; i++) {
          ...
          if (lp->states > 0) {
            scheduler->state->stepType = ST_State;
#endif
            BT_UPDATE_TREE(scheduler, states, simData->SD[j], simData->nSD[j], update);
#ifdef QSS_PARALLEL
          }
#endif
        }
    ...
    scheduler->state->stepType = ST_Event;
#endif
    BT_UPDATE_TREE(scheduler, events, simData->HZ[stateVar], simData->nHZ[stateVar], update);
    if (simTime->nextEventTime[events->nodes[events->height][0].value] < minVal) {
      minIdx = events->nodes[events->height][0].value;
      minVal = simTime->nextEventTime[minIdx];
      w = ST_Event;
...
#ifdef QSS_PARALLEL
void BT_PAR_updateI(SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
#else
void BT_updateI(SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
#endif
{
  BT_tree states = scheduler->state->states;
...
#ifdef QSS_PARALLEL
void BT_PAR_updateHI(SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
#else
void BT_updateHI(SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
#endif
{
  int w = ST_State, minIdx = -1, stateVar, update, nStates = simData->states;
  double minVal = INF;
...
#endif
{
...
#endif
#ifdef QSS_PARALLEL
  scheduler->state->states = BT_Tree(lp->states, simTime->nextStateTime, simData->states, lp->qOutMap);
#else
  scheduler->state->states = BT_Tree(simData->states, simTime->nextStateTime, simData->states, NULL);
#endif
  if (nStates) {
  ...
  if (nEvents) {
#ifdef QSS_PARALLEL
    scheduler->state->events = BT_Tree(lp->events, simTime->nextEventTime, simData->events, lp->eOutMap);
#else
    scheduler->state->events = BT_Tree(simData->events, simTime->nextEventTime, simData->events, NULL);
#endif
    BT_INIT_TREE(scheduler->state->events);
    if (simTime->nextEventTime[scheduler->state->events->nodes[scheduler->state->events->height][0].value] < minVal) {
      minIdx = scheduler->state->events->nodes[scheduler->state->events->height][0].value;
      minVal = simTime->nextEventTime[minIdx];
      w = ST_Event;
    }
  }
  if (nInputs) {
#ifdef QSS_PARALLEL
    scheduler->state->inputs = BT_Tree(lp->inputs, simTime->nextInputTime, simData->inputs, lp->iMap);
#else
    scheduler->state->inputs = BT_Tree(simData->inputs, simTime->nextInputTime, simData->inputs, NULL);
#endif
    BT_INIT_TREE(scheduler->state->inputs);
    if (simTime->nextInputTime[scheduler->state->inputs->nodes[scheduler->state->inputs->height][0].value] < minVal) {
      minIdx = scheduler->state->inputs->nodes[scheduler->state->inputs->height][0].value;
      minVal = simTime->nextInputTime[minIdx];
      w = ST_Input;
    }
  }
