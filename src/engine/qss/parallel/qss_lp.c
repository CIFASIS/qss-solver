...
void LP_freePartitionInfo(LP_partitionInfo pi)
{
  free(pi->stateInput);
...
LP_partitionInfo *LP_PartitionInfoArray(int lps, int states, int handlers, int out)
{
...
void LP_freePartitionInfoArray(LP_partitionInfo *pi, int lps)
{
  int i;
  for (i = 0; i < lps; i++) {
...
bool LP_find(int *vector, int size, int value)
{
  int i;
  for (i = 0; i < size; i++) {
    if (vector[i] == value) {
...
LP_partitionInfo *LP_processPartition(QSS_simulator simulator, PRT_partition partition)
{
  /* Local mappings */
...
void LP_initializeDataStructs(QSS_simulator simulator, PRT_partition partition)
{
  /* Local mappings */
  QSS_data simulatorData = simulator->data;
  ...
  bool autonomous;
  for (i = 0; i < lps; i++) {
    simulator->lps->lp[i] = QSS_LP_Data(pi[i]->states, pi[i]->handlers, 0, pi[i]->outputs, pi[i]->stateInputs, pi[i]->handlerInputs, states,
                                        events, outputs, pi[i]->nLPS, pi[i]->lps, i);
  }
  ...
  for (i = beginHandlers; i < endHandlers; i++) {
    ...
    if (nOutputs > 0) {
      ...
      simulator->lps->lp[partitionNumber]->eMap[handlerNumber] = ASSIGNED;
    }
