...
grp_t getSteps(QSS_simulator simulator, grp_t infVar)
{
...
void QSS_generateWeights(QSS_simulator simulator)
{
...
{
  SD_print(simulator->simulationLog, "Simulation time: %g ms", simulator->stats->simulationTime);
  SD_print(simulator->simulationLog, "CPU time per transition: %g", simulator->stats->simulationTime / simulator->stats->totalSteps);
  int nOutputs = simulator->output->outputs;
  if (nOutputs > 0) {
    SD_print(simulator->simulationLog, "Simulation output:");
    int j;
    for (j = 0; j < nOutputs; j++) {
      SD_print(simulator->simulationLog, "Variable %s changes: %d", simulator->output->variable[j].name, OUT_getSteps(simulator->log, j));
    }
  }
  SD_print(simulator->simulationLog, "Simulation transitions: %lu", simulator->stats->totalSteps);
  if (simulator->stats->reinits > 0) {
    SD_print(simulator->simulationLog, "State variable reinits: %lu", simulator->stats->reinits);
  }
  SD_print(simulator->simulationLog, "Initialization time: %g ms", simulator->stats->initTime);
  SD_print(simulator->simulationLog, "Save data time: %g ms", simulator->stats->saveTime);
...
void QSS_SEQ_logMemory(QSS_simulator simulator)
{
  ...
  int xOrder = data->order + 1, states = data->states, events = data->events, sDouble = sizeof(double), sInt = sizeof(int),
      discretes = data->discretes, outputs = output->outputs, algs = data->algs, inputs = data->inputs;
  simulator->stats->memory += (3 * states + 4 * states * xOrder + algs * xOrder + discretes) * sDouble + (4 * states + inputs) * sInt;
  if (events) {
  ...
  if (outputs) {
    ...
    if (nOutputs > 0) {
      ...
      for (j = 0; j < nOutputs; j++) {
        ...
        simulator->stats->memory += (2 * data->params->nodeSize + nodes * data->params->nodeSize * 2) * sDouble;
      }
    }
  }
...
void QSS_SEQ_initialize(SIM_simulator simulate)
{
...
#ifdef DEBUG
    if (settings->debug & SD_DBG_InitValues) {
      SD_print(simulationLog, "Initial derivative: x[%d][1] = %g", i, x[i * coeffs + 1]);
    }
#endif
  }
#ifdef DEBUG
  ...
  for (i = 0; i < forUL; i++) {
    ...
    qssModel->events->zeroCrossing(i, q, qssData->d, qssData->alg, qssTime->time, zc);
    s = sign(zc[0]);
  ...
  forUL = qssData->states;
  for (i = 0; i < forUL; i++) {
    QA_recomputeNextTime(quantizer, i, qssTime->time, qssTime->nextStateTime, x, qssData->lqu, q);
  }
#ifdef DEBUG
  if (settings->debug & SD_DBG_InitValues) {
    SD_print(simulationLog, "done.");
...
{
  SD_print(simulator->simulationLog, "Simulation Results for LP: %d", simulator->id);
  SD_print(simulator->simulationLog, "");
  SD_print(simulator->simulationLog, "Simulation time: %g ms", simulator->stats->simulationTime);
  SD_print(simulator->simulationLog, "CPU time per transition: %g", simulator->stats->simulationTime / simulator->stats->totalSteps);
    ...
    if (nOutputs > 0) {
      ...
      for (j = 0; j < nOutputs; j++) {
        if (simulator->data->lp->oMap[j] > NOT_ASSIGNED) {
          SD_print(simulator->simulationLog, "Variable %s changes: %d", simulator->output->variable[j].name,
  ...
  SD_print(simulator->simulationLog, "Simulation messages sent: %lu", simulator->stats->messages);
  SD_print(simulator->simulationLog, "Simulation external events: %lu", simulator->stats->extTrans);
  SD_print(simulator->simulationLog, "Simulation past events: %lu (%.2f %%)", simulator->stats->pastEvents,
           (simulator->stats->extTrans > 0) ? ((double)simulator->stats->pastEvents / (double)simulator->stats->extTrans) * 100 : 0);
  SD_print(simulator->simulationLog, "Dt sum: %g", simulator->dt->state->avgDt);
  SD_print(simulator->simulationLog, "Dt changes: %d", simulator->dt->state->dtChanges);
  if (simulator->dt->state->dtChanges > 0) {
    SD_print(simulator->simulationLog, "Average dt: %g", simulator->dt->state->avgDt / simulator->dt->state->dtChanges);
  } else {
...
{
  unsigned long totalSimSteps = 0, totalLPSteps = 0, maxSteps = simulator->stats->steps[0], minSteps = maxSteps, totalExternalEvents = 0,
                totalMessages = 0;
  ...
  for (i = 0; i < lps; i++) {
    int simSteps = simulator->stats->steps[i] + simulator->stats->simulationExternalEvents[i];
  ...
  SD_print(simulator->simulationLog, "");
  SD_print(simulator->simulationLog, "Simulation time: %g ms", maxSimulationTime);
  SD_print(simulator->simulationLog, "Total Simulation transitions: %lu", totalLPSteps);
  SD_print(simulator->simulationLog, "Initialization time: %g ms", simulator->stats->initTime);
  SD_print(simulator->simulationLog, "Partitioning time: %g ms", simulator->stats->partitioningTime);
  SD_print(simulator->simulationLog, "Initialize LPS time: %g ms", simulator->stats->initializeLPS);
  SD_print(simulator->simulationLog, "Messages sent: %lu", totalMessages);
  SD_print(simulator->simulationLog, "External events: %lu", totalExternalEvents);
  SD_print(simulator->simulationLog, "Processed messages: %.2f %%",
           (totalMessages > 0) ? ((double)totalExternalEvents / (double)totalMessages) * 100 : 0);
  SD_print(simulator->simulationLog, "Estimated load (time): %g", maxSimulationTime / avgTime);
  SD_print(simulator->simulationLog, "Estimated max load (steps): %g", maxSteps / avgSteps);
  SD_print(simulator->simulationLog, "Estimated min load (steps): %g", minSteps / avgSteps);
  ...
  SD_print(simulator->simulationLog, "Additional memory allocated: %.2lf MBytes (%.2f %%)", memoryIncrement * sm,
           (memoryIncrement / (double)lps) * 100);
#ifdef DEBUG
  QSS_generateWeights(simulator);
#endif
  SD_print(simulator->simulationLog, "");
...
void QSS_PAR_printParallelLog(QSS_simulator simulator, int code)
{
  ...
  case PAR_ERR_SET_AFFINITY_MASK:
    SD_print(simulator->simulationLog, "LP %d: WARNING: can not set affinity mask\n", simulator->id);
    break;
  case PAR_ERR_GET_AFFINITY_MASK:
    SD_print(simulator->simulationLog, "LP %d: WARNING: can not get affinity mask\n", simulator->id);
    break;
  default:
    break;
  }
...
QSS_simulator QSS_PAR_copySimulator(QSS_simulatorInstance *instance)
{
  ...
  p->time = QSS_Time(data->states, data->events, data->inputs, data->it, root->time->scheduler, root->time->weights);
...
void QSS_PAR_controlPassiveLPS(QSS_simulator simulator)
{
  ...
  int lps = simulator->data->params->lps;
  for (i = 0; i < lps; i++) {
    if (simulator->lpTime[i] != simulator->data->ft && simulator->lpTime[i] != INF) {
      return;
    }
  }
  for (i = 0; i < lps; i++) {
...
void QSS_PAR_logMemory(QSS_simulator simulator)
{
  ...
  for (i = 0; i < lps; i++) {
    ...
    simulator->stats->memory += (lp->totalStates + lp->totalEvents + lp->totalOutputs + lp->states + lp->events + lp->inputs + lp->outputs +
                                 lp->inStates + lp->inEvents) *
                                sInt;
    simulator->stats->memory += (4 * states * xOrder + states + algs * xOrder + discretes) * sDouble;
    int treeH = 0, treeS = 0, treeE = 0, treeI = 0, max = states, treeT = 0;
    if (lp->states) {
  ...
  simulator->stats->memory += (states * sDouble + 64 * sMessage + 10000 * sMessage + 8) * lps;
}
...
void QSS_PAR_initialize(SIM_simulator simulate)
{
  ...
  getTime(simulator->stats->iTime);
  PRT_partition partition = PRT_Partition(simulator->data, simulator->output->name);
  getTime(simulator->stats->sdTime);
...
void QSS_PAR_sendSimultaneousMessages(QSS_simulator simulator, QSS_externaEventHandler externalEvent,
                         ...
                         QSS_internalEventHandler internalEvent)
{
  ...
  if (settings->debug & SD_DBG_Synchronize) {
    SD_print(simulationLog, "LP %d synchronize variable = %d, type = %d, localTime = %.16lf, globalTime = %.16lf", id, qssTime->minIndex,
             qssTime->type, qssTime->time, simulator->lpTime[id]);
  }
#endif
...
#else
void QSS_PAR_saveLog(QSS_simulator simulator) { return; }

void QSS_PAR_printSimulationLog(QSS_simulator simulator) { return; }

void QSS_PAR_printParallelLog(QSS_simulator simulator, int code) { return; }

void QSS_PAR_allocRootSimulatorData(QSS_simulator simulator) { return; }

QSS_simulator QSS_PAR_copySimulator(QSS_simulatorInstance *instance) { return NULL; }

double QSS_PAR_GVT(QSS_simulator simulator) { return 0; }

void QSS_PAR_removePendingMessages(QSS_simulator simulator) { return; }

void QSS_PAR_controlPassiveLPS(QSS_simulator simulator) { return; }

void QSS_PAR_initializeSimulation(QSS_simulator simulator) { return; }

void QSS_PAR_logMemory(QSS_simulator simulator) { return; }

void QSS_PAR_initialize(SIM_simulator simulate) { return; }
