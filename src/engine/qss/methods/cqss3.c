...
void QSS_FUNC_DECL(CQSS3, init)(QA_quantizer quantizer, QSS_data simData, QSS_time simTime)
{
  ...
  QSS_ASSIGN_QUANTIZER_OPS(CQSS3)
}

void QSS_FUNC_DECL(CQSS3, recomputeNextTime)(QA_quantizer quantizer, int var, double t, double *nTime, double *x, double *lqu, double *q)
{
...
void QSS_FUNC_DECL(CQSS3, recomputeNextTimes)(QA_quantizer quantizer, int vars, int *inf, double t, double *nTime, double *x, double *lqu,
                                              double *q)
{
  int i;
  ...
  for (i = 0; i < vars; i++) {
    QSS_PARALLEL_EXP(if (map[inf[i]] != NOT_ASSIGNED) {)
      QSS_FUNC_INVK(CQSS3, recomputeNextTime)(quantizer, inf[i], t, nTime, x, lqu, q);
    QSS_PARALLEL_EXP(
    })
  }
}
...
#ifdef QSS_PARALLEL
void CQSS3_PAR_nextTime(QA_quantizer quantizer, int var, double t, double *nTime, double *x, double *lqu)
#else
void CQSS3_nextTime(QA_quantizer quantizer, int var, double t, double *nTime, double *x, double *lqu)
#endif
{
  int cf3 = var * 4 + 3;
  if (x[cf3] == 0) {
...
void QSS_FUNC_DECL(CQSS3, updateQuantizedState)(QA_quantizer quantizer, int var, double *q, double *x, double *lqu)
{
  int cf0 = var * 4, cf1 = cf0 + 1, cf2 = cf1 + 1, cf3 = cf2 + 1;
  double dx, elapsed;
  double *u0 = quantizer->state->u0;
