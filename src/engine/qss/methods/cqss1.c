...
void QSS_FUNC_DECL(CQSS1, init)(QA_quantizer quantizer, QSS_data simData, QSS_time simTime)
{
  int i;
  ...
  QSS_ASSIGN_QUANTIZER_OPS(CQSS1)
}

void QSS_FUNC_DECL(CQSS1, recomputeNextTimes)(QA_quantizer quantizer, int vars, int *inf, double t, double *nTime, double *x, double *lqu,
                                              double *q)
{
  int i;
  ...
  for (i = 0; i < vars; i++) {
    QSS_PARALLEL_EXP(if (map[inf[i]] != NOT_ASSIGNED) {)
      QSS_FUNC_INVK(CQSS1, recomputeNextTime)(quantizer, inf[i], t, nTime, x, lqu, q);
    QSS_PARALLEL_EXP(
    })
  }
}

void QSS_FUNC_DECL(CQSS1, recomputeNextTime)(QA_quantizer quantizer, int var, double t, double *nTime, double *x, double *lqu, double *q)
{
...
void QSS_FUNC_DECL(CQSS1, nextTime)(QA_quantizer quantizer, int var, double t, double *nTime, double *x, double *lqu)
{
  int cf1 = var * 2 + 1;
  if (x[cf1] == 0) {
    nTime[var] = INF;
...
void QSS_FUNC_DECL(CQSS1, updateQuantizedState)(QA_quantizer quantizer, int var, double *q, double *x, double *lqu)
{
  int cf0 = var * 2, cf1 = cf0 + 1;
  double dx = 0;
  double *u = quantizer->state->u0;
