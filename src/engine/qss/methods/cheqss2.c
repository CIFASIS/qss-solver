...
void QSS_FUNC_DECL(CHEQSS2, init)(QA_quantizer quantizer, QSS_data simData, QSS_time simTime)
{
  ...
  QSS_ASSIGN_QUANTIZER_OPS(CHEQSS2)
...
void QSS_FUNC_DECL(CHEQSS2, recomputeNextTime)(QA_quantizer quantizer, int var, double t, double *nTime, double *x, double *lqu, double *q)
{
...
void QSS_FUNC_DECL(CHEQSS2, recomputeNextTimes)(QA_quantizer quantizer, int vars, int *inf, double t, double *nTime, double *x, double *lqu,
                                                double *q)
{
  int i;
  ...
  for (i = 0; i < vars; i++) {
    QSS_PARALLEL_EXP(if (map[inf[i]] != NOT_ASSIGNED) {)
      QSS_FUNC_INVK(CHEQSS2, recomputeNextTime)(quantizer, inf[i], t, nTime, x, lqu, q);
    QSS_PARALLEL_EXP(
    })
  }
}

void QSS_FUNC_DECL(CHEQSS2, nextTime)(QA_quantizer quantizer, int var, double t, double *nTime, double *x, double *lqu)
{
  int cf2 = var * 3 + 2;
  if (x[cf2] == 0)
    nTime[var] = INF;
  else
    nTime[var] = t + sqrt(8 * fabs(lqu[var] / x[cf2]));
}

void QSS_FUNC_DECL(CHEQSS2, updateQuantizedState)(QA_quantizer quantizer, int var, double *q, double *x, double *lqu)
{
  ...
  if (quantizer->state->flag2[var] == 1 && t > 0) {
    ...
    if (fabs(disc) < a[var] * a[var] * lqu[var]) {
      ...
      double tm2 = 1e20;
      if (a[var] == 0) {
        if (disc) tm2 = sqrt(16 * lqu[var] / fabs(disc));
      } else {
        double c2 = -fabs(disc) / lqu[var] + a[var] * a[var];
        tm2 = 4 * fabs((a[var] - sqrt(a[var] * a[var] - c2)) / c2);
      }
      q[cf0] = x[cf0] - sign * lqu[var];
      q[cf1] = a[var] * q[cf0] + u0[var] + 8 / tm2 * sign * lqu[var];
    }
  }
}
