...
void QSS_FUNC_DECL(CHEQSS1, init)(QA_quantizer quantizer, QSS_data simData, QSS_time simTime)
{
  ...
  QSS_ASSIGN_QUANTIZER_OPS(CHEQSS1)
...
void QSS_FUNC_DECL(CHEQSS1, recomputeNextTime)(QA_quantizer quantizer, int var, double t, double *nTime, double *x, double *lqu, double *q)
{
  ...
  if (quantizer->state->flag2[var] == 1) {
    ...
    if (x[cf1] != 0) {
      dt1 = (q[cf0] + 1.001 * lqu[var] * sign - x[cf0]) / x[cf1];
      if (dt1 > 0) {
        nTime[var] = t + dt1;
      } else {
  ...
  //     printf("recomputeNextTime: t=%g: q[%d]=%g, x=%g, dx=%g, nTime=%g \n",t,var,q[cf0],x[cf0],x[cf1],nTime[var]);
}

void QSS_FUNC_DECL(CHEQSS1, recomputeNextTimes)(QA_quantizer quantizer, int vars, int *inf, double t, double *nTime, double *x, double *lqu,
                                                double *q)
{
  int i;
  ...
  for (i = 0; i < vars; i++) {
    QSS_PARALLEL_EXP(if (map[inf[i]] != NOT_ASSIGNED) {)
      QSS_FUNC_INVK(CHEQSS1, recomputeNextTime)(quantizer, inf[i], t, nTime, x, lqu, q);
    QSS_PARALLEL_EXP(
    })
  }
}

void QSS_FUNC_DECL(CHEQSS1, nextTime)(QA_quantizer quantizer, int var, double t, double *nTime, double *x, double *lqu)
{
  int cf1 = var * 2 + 1;
  if (x[cf1] == 0) {
    nTime[var] = INF;
...
void QSS_FUNC_DECL(CHEQSS1, updateQuantizedState)(QA_quantizer quantizer, int var, double *q, double *x, double *lqu)
{
