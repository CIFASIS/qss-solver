...
void QSS_FUNC_DECL(EQSS3, init)(QA_quantizer quantizer, QSS_data simData, QSS_time simTime)
{
  ...
  QSS_ASSIGN_QUANTIZER_OPS(EQSS3)
...
void QSS_FUNC_DECL(EQSS3, recomputeNextTime)(QA_quantizer quantizer, int var, double t, double *nTime, double *x, double *lqu, double *q)
{
...
void QSS_FUNC_DECL(EQSS3, recomputeNextTimes)(QA_quantizer quantizer, int vars, int *inf, double t, double *nTime, double *x, double *lqu,
                                              double *q)
{
  int i;
  ...
  for (i = 0; i < vars; i++) {
    QSS_PARALLEL_EXP(if (map[inf[i]] != NOT_ASSIGNED) {)
      QSS_FUNC_INVK(EQSS3, recomputeNextTime)(quantizer, inf[i], t, nTime, x, lqu, q);
    QSS_PARALLEL_EXP(
    })
  }
}

void QSS_FUNC_DECL(EQSS3, nextTime)(QA_quantizer quantizer, int var, double t, double *nTime, double *x, double *lqu)
{
  int cf3 = var * 4 + 3;
  if (x[cf3] == 0) {
    nTime[var] = INF;
  } else {
    nTime[var] = t + pow(fabs(8 * lqu[var] / x[cf3]), 1.0 / 3);
  }
}

void QSS_FUNC_DECL(EQSS3, updateQuantizedState)(QA_quantizer quantizer, int var, double *q, double *x, double *lqu)
{
  ...
  if (quantizer->state->flag2[var] == 1) {
    ...
    if (fabs(disc) < fabs(a[var] * a[var] * a[var]) * lqu[var]) {
      ...
      if (a[var] == 0) {
        if (disc) tm3 = pow(6 * lqu[var] / fabs(disc), 1.0 / 3);
      } else {
        coeff[0] = 6;
        coeff[1] = -6 * a[var];
        coeff[2] = 3 * a[var] * a[var];
        coeff[3] = -fabs(disc) / lqu[var] - a[var] * a[var] * a[var];
        tm3 = minPosRoot(coeff, 3);
      }
      double p0 = -sign * lqu[var];
      q[cf0] = x[cf0] - p0;
      q[cf1] = a[var] * q[cf0] + u0[var] + 3 * p0 / tm3;
      q[cf2] = (a[var] * q[cf1] + u1[var] - 6 * p0 / (tm3 * tm3)) / 2;
    }
  }
}
