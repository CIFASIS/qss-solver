...
void QSS_FUNC_DECL(EQSS2, init)(QA_quantizer quantizer, QSS_data simData, QSS_time simTime)
{
  ...
  QSS_ASSIGN_QUANTIZER_OPS(EQSS2)
...
void QSS_FUNC_DECL(EQSS2, recomputeNextTime)(QA_quantizer quantizer, int var, double t, double *nTime, double *x, double *lqu, double *q)
{
...
void QSS_FUNC_DECL(EQSS2, recomputeNextTimes)(QA_quantizer quantizer, int vars, int *inf, double t, double *nTime, double *x, double *lqu,
                                              double *q)
{
  int i;
  ...
  for (i = 0; i < vars; i++) {
    QSS_PARALLEL_EXP(if (map[inf[i]] != NOT_ASSIGNED) {)
      QSS_FUNC_INVK(EQSS2, recomputeNextTime)(quantizer, inf[i], t, nTime, x, lqu, q);
    QSS_PARALLEL_EXP(
    })
  }
}

void QSS_FUNC_DECL(EQSS2, nextTime)(QA_quantizer quantizer, int var, double t, double *nTime, double *x, double *lqu)
{
  int cf2 = var * 3 + 2;
  if (x[cf2] == 0)
    nTime[var] = INF;
  else
    nTime[var] = t + sqrt(4 * fabs(lqu[var] / x[cf2]));
}

void QSS_FUNC_DECL(EQSS2, updateQuantizedState)(QA_quantizer quantizer, int var, double *q, double *x, double *lqu)
{
