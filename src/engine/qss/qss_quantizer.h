 ...
 * @brief `sim_step` is a boolean array that represents whether a simultaneous step should be taken in mLIQSS2 methods given that the value
 * j-th quantized state was updated during the i-th quantized state updated to avoid oscilations.
 *
 * @var QA_quantizerState_::A
 ...
 * @brief In mLIQSS2 methods, qj is an array that contains the value of the j-th variable updated due to a change in the i-th variable. This
 * value is needed to interact with the QSS engine. When an oscilation is detected, `sim_step` property is set for j and when the
 * derivatives are updated it will set a next state change for the current simulation time that will update the second state variable.
 */
struct QA_quantizerState_ {
  int order;
  int xOrder;
