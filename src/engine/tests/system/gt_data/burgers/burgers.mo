model burgers
	...
	for i in 1:N loop
		x[i]:=1.0*i/N;
	//	u[i]:=sin(pi*2*x[i])+2; //Condici�n inicial que mantiene u>0 en todo el dominio
		u[i]:=sin(pi*2*x[i]); //Condici�n inicial con u<0	 //Sin la suma convexa, el esquema falla.
      if u[i] > 0 then
          ap[i]:=1;    
      else
      ap[i]:=0;
	...
	equation
//Eq. Burgers inv�scida: u_t+(1/2)*(u^2)_x=0
//T�rmino viscoso: eps*uxx (t�rmino fuente a la derecha)
//Forma cuasi-lineal: u_t+u*u_x=0		(lamb=u)
//La velocidad de propagaci�n es igual a la variable.
//Upwind:
//		  der(u[1])=(u[1]+u[N])/2*(-u[1]+u[N])*N;	//Condici�n peri�dica para u>0
...
//Lax-Wendroff: esquema 2 orden: Problema para implementarlo: necesito afectar el avance temporal con elementos de la discretizaci�n espacial
	annotation(
