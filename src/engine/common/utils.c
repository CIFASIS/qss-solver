...
void cleanVector(int *vector, int value, int size) { memset(vector, value, size * sizeof(int)); }

void cleanDoubleVector(double *vector, int value, int size) { memset(vector, value, size * sizeof(double)); }
...
double minPosRoot(double *coeff, int order)
{
  ...
  case 3:
    if ((coeff[3] == 0) || (1000 * fabs(coeff[3]) < fabs(coeff[2]))) {
      ...
      r = (9 * coeff[3] * coeff[2] * coeff[1] - 27 * coeff[3] * coeff[3] * coeff[0] - 2 * coeff[2] * coeff[2] * coeff[2]) / 54 / coeff[3] /
          coeff[3] / coeff[3];
      disc = q * q * q + r * r;
      mpr = INF;
      if (disc >= 0) {
...
void integrateState(int i, double dt, double *p, int order)
{
  int i0 = i;
  switch (order) {
  case 1: {
  ...
  case 4: {
    ...
    p[i0] = p[i0] + dt * p[i1] + dt * dt * p[i2] + dt * dt * dt * p[i3] + dt * dt * dt * dt * p[i4];
    p[i1] = p[i1] + 2 * p[i2] * dt + 3 * p[i3] * dt * dt + 4 * p[i4] * dt * dt * dt;
...
double evaluatePoly(int i, double dt, double *p, int order)
{
  int i0 = i;
  switch (order) {
  case 0:
  ...
  case 3: {
    return p[i0] + dt * p[i0 + 1] + dt * dt * p[i0 + 2] + dt * dt * dt * p[i0 + 3];
  }
  case 4: {
    return p[i0] + dt * p[i0 + 1] + dt * dt * p[i0 + 2] + dt * dt * dt * p[i0 + 3] + dt * dt * dt * dt * p[i0 + 4];
  }
  }
  return INF;
}
...
double evaluateVectorPoly(double dt, double *p, int order)
{
  ...
  case 4:
    return p[0] + dt * p[1] + dt * dt * p[2] + dt * dt * dt * p[3] + dt * dt * dt * dt * p[4];
  }
  return INF;
}

node Node(int ns, int ord)
{
  int i;
...
int vectorCompare(const void *a, const void *b) { return *(int *)a - *(int *)b; }
...
bool vectorEnd(vector v) { return v->iter >= v->used; }
...
#ifdef SYNC_RT
struct timeval init;
void setInitRealTime() { gettimeofday(&init, NULL); }
...
void BIT_set(BIT_vector b, int bit) { b->words[bit >> 5] |= 1 << (bit % BITS_PER_WORD); }

void BIT_clear(BIT_vector b, int bit) { b->words[bit >> 5] &= ~(1 << (bit % BITS_PER_WORD)); }

unsigned long BIT_isSet(BIT_vector b, int bit) { return b->words[bit >> 5] & (1 << (bit % BITS_PER_WORD)); }
...
word_t BIT_isAnySet(BIT_vector b) { return (b->words[0] & 0xFFFFFFFF) || (b->words[1] & 0xFFFFFFFF); }

word_t BIT_setBits(BIT_vector b) { return BIT_numberOfSetBits(b->words[0]) + BIT_numberOfSetBits(b->words[1]); }

void BIT_setMask(BIT_vector b, int bit) { b->resetMask[bit >> 5] |= 1 << (bit % BITS_PER_WORD); }
...
IBX_inbox IBX_Inbox(int states, int ack, int *qMap)
{
  ...
  if (ack == 0) {
    p->messages = (IBX_message *)checkedMalloc(MAX_LPS * sizeof(IBX_message));
    p->orderedMessages = (IBX_message *)checkedMalloc(BUFFER_SIZE * sizeof(IBX_message));
    int i;
    for (i = 0; i < MAX_LPS; i++) {
...
void IBX_add(MLB_mailbox mailbox, IBX_inbox inbox, int from, int id, IBX_message message)
{
...
int IBX_compare(const void *a, const void *b)
{
...
void IBX_receiveMessages(IBX_inbox inbox)
{
  ...
  int tail = inbox->tail;
  int size = inbox->size;
  for (j = BIT_first(inbox->received); j < MAX_LPS; j = BIT_next(inbox->received)) {
...
void IBX_receiveAndAckMessages(IBX_inbox inbox, MLB_mailbox mailbox, int id)
{
  ...
  inbox->head = 0;
  if (inbox->waitingAck != NULL) {
    for (j = BIT_first(inbox->waitingAck); j < states; j = BIT_next(inbox->waitingAck)) {
...
IBX_message IBX_nextMessage(IBX_inbox inbox)
{
  pthread_mutex_lock(&(inbox->receivedMutex));
  int head = inbox->head;
...
double IBX_nextMessageTime(IBX_inbox inbox) { return inbox->orderedMessages[inbox->head].time; }
...
void MLB_ack(MLB_mailbox mailbox, int to, int from) { IBX_ack(mailbox->inbox[MSG_ACK][to], from); }

#else

MLB_mailbox MLB_Mailbox(int lps) { return NULL; }

void MLB_freeMailbox(MLB_mailbox mailbox) { return; }

void MLB_send(MLB_mailbox mailbox, int to, int from, IBX_message message) { return; }

void MLB_ack(MLB_mailbox mailbox, int to, int from) { return; }

void MLB_close(MLB_mailbox mailbox, int to, int dir) { return; }
