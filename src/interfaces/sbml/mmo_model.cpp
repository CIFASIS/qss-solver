    ...
    for(it = eq.firstEvent(); !eq.endEvent(); it = eq.nextEvent())
    {
      ...
      for(zc_it = zc.firstZC(); !zc.endZC(); zc_it = zc.nextZC())
      {
        ...
        for(asg_it = it.second.begin(); asg_it != it.second.end(); asg_it++)
        {
          ...
          MMODecl asg = MMODecl(asgVar, asg_exp.getExp(), DEC_ASSIGNMENT,
              asg_exp.isConditional());
...
MMOModel::_addAsgs(const Event &x, MMOMath zc, int pos, MMOEvent *ev)
{
  ...
  for(int i = 0; i < asg_size; i++)
  {
    ...
    {
      asg_dec = MMODecl(asg_var, as.getExp(), DEC_ASSIGNMENT,
          as.isConditional());
...
MMOModel::add(const Event &x)
{
  ...
  for(zc_def = zc.firstZC(); !zc.endZC(); zc_def = zc.nextZC())
  {
    ...
    {
      MMODecl asg_dec;
      asg_dec = MMODecl(delay_var_str, delay_as.getExp(), DEC_ASSIGNMENT,
          delay_as.isConditional());
      ev->add(asg_dec, positive);
    }
    else
    {
...
MMOModel::_reorder()
{
  ...
  for(expIt = _constantAlgebraics.begin(); expIt != _constantAlgebraics.end();
      expIt++)
  {
...
MMOModel::add(const FunctionDefinition &f)
{
  ...
  func->add(adec);
  _functions[fname] = pair<list<string>, ASTNode*>(funcArgs,
      new ASTNode(*f.getBody()));
