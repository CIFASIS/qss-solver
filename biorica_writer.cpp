...
BioRicaWriter::~BioRicaWriter ()
{
    ...
    if (!_nodeFieldTransitions.empty ())
    {
        ...
        for (it = _nodeFieldTransitions.begin (); it != _nodeFieldTransitions.end (); it++)
        {
...
BioRicaWriter::visit (MMODecl *x)
{
    ...
    if (x->isDerivative ())
    {
        _nodeFieldEqDiffs.push_back (utils->indent (_indent) + "d" + _flatId (x) + " = " + x->getExp () + ";");
    }
    else if (x->isAlgebraicEquation ())
    {
        _formulas.push_back (utils->indent (_indent) + "formula " + _flatId (x) + " = " + x->getExp () + ";");
    }
    else if (x->isInitialAssignment ())
    {
        _nodeFieldInits[_flatId (x)] = utils->indent (_indent) + _flatId (x) + " := " + x->getExp ();
    }
    else if (x->isAssignment () || x->isReinit ())
    {
        pair<string, list<string>*> ev = _nodeFieldTransitions[_currentEvent];
        ev.second->push_back (utils->indent (_indent) + _flatId (x) + " := " + x->getExp ());
    }
    ...
    {
        _nodeElementStates.push_back (utils->indent (_indent) + _flatId (x) + ":FLOAT;");
        if (x->hasValue ())
        {
            stringstream buff;
    ...
    else if (x->isParameter ())
    {
        ...
        {
            stringstream buff;
            buff << utils->indent (_indent) << "const " << _flatId (x) << " = " << x->getValue () << ";";
