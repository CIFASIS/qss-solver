...
Index_::Index_ (int constant, int factor, int low, int hi) :
        _constant (constant), _factor (factor), _offset (0), _low (low), _hi (hi), _isSet (true), _array (false), _map (NULL), _parameter (), _rangeOp (
...
Index_::Index_ (int constant, int factor) :
        _constant (constant), _factor (factor), _offset (0), _low (1), _hi (1), _isSet (true), _array (false), _map (NULL), _parameter (), _rangeOp (
                false), _hasRange (false)
{
    _constant--;
}
...
Index_::intersection (Index_ b) const
{
    ...
    if (hasRange () && b.hasRange ())
    {
        ...
        else if (b.lowValue () < lowValue () && lowValue () <= b.hiValue () && b.hiValue () < hiValue ())
        {
            if (!equalExp (b) && !getIntersection (b))
            {
                is.setType (IDX_DISJOINT);
        ...
        else if (lowValue () < b.lowValue () && b.lowValue () <= hiValue () && hiValue () < b.hiValue ())
        {
            if (!equalExp (b) && !getIntersection (b))
            {
                is.setType (IDX_DISJOINT);
...
Index_::operator == (const Index_& other) const
{
    return (this->_constant == other._constant && this->_factor == other._factor && this->_hasRange == other._hasRange && this->_hi == other._hi
            && this->_index == other._index && this->_isSet == other._isSet && this->_low == other._low && this->_offset == other._offset
            && this->_map == other._map && this->_parameter == other._parameter && this->_rangeOp == other._rangeOp && this->_array == other._array);
}
