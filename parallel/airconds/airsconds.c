...
MOD_definition(int i, double *x, double *d, double *alg, double t, double *dx)
{
	...
	switch(i)
	{
		...
		default:
			...
			if(j >=0 && j <= 1999)
			{
				dx[1] = (__PAR_THA/__PAR_RES[(j)]-__PAR_POT[(j)]*d[(j+2001)]-x[(j) * 4]/__PAR_RES[(j)]+d[(j+4004)]/__PAR_RES[(j)])/__PAR_CAP[(j)];
...
{
		der[(j) * 4 + 1] = (__PAR_THA/__PAR_RES[(j)]-__PAR_POT[(j)]*d[(j+2001)]-x[(j) * 4]/__PAR_RES[(j)]+d[(j+4004)]/__PAR_RES[(j)])/__PAR_CAP[(j)];
