...
AST_MicroModelica_::AST_MicroModelica_ (string name) :
        _className (NULL), _fatherName (NULL), _childName (NULL), _classModification (false),
    ...
    if (_classPrefix == CP_FUNCTION || _classPrefix == CP_PURE || _classPrefix == CP_IMPURE)
    {
        if (x->hasCompositionList ())
        {
            ...
            foreach(it,cl)
            {
                ...
                {
                    Error::getInstance ()->add (x->lineNum (), EM_AST | EM_CLASS_DEFINITION, ER_Error,
...
AST_MicroModelica_::visit (AST_External_Function_Call x)
{
    ...
    {
        Error::getInstance ()->add (x->lineNum (), EM_AST | EM_CLASS_DEFINITION, ER_Error, "Language Specification not recognized.");
    }
}
    ...
    if (_classPrefix == CP_PACKAGE)
    {
        if (e == ELCLASS)
        {
            ...
            if (!(c & CP_FUNCTION) && !(c & CP_PURE) && !(c & CP_IMPURE))
            {
                Error::getInstance ()->add (x->lineNum (),
                EM_AST | EM_CLASS_DEFINITION,
                                            ER_Error, "Only function class definitions allowed inside Package class.");
            }
        }
    }
    if (e == ELCLASS)
