...
MMO_Statement_::MMO_Statement_ (AST_Statement stm, MMO_ModelData data) :
        _stm (stm), _data (data), _deps (NULL), _lhs (NULL), _initialCode (data->initialCode ()), _variables (), _eventLhs (_data->lhs ())
{
    if (_initialCode)
    {
            ...
            if (sa->exp ()->expressionType () == EXPCALLARG)
            {
                switch (Util::getInstance ()->checkBuiltInFunctions (name))
                {
                    case BIF_REINIT:
                    {
                        ...
                        {
                            Error::getInstance ()->add (eca->lineNum (), EM_IR | EM_WRONG_EXP, ER_Error, "Expected 2 arguments, found %d",
                                                        eca->arguments ()->size ());
                        }
                        ...
                        if (e->expressionType () != EXPCOMPREF)
                        {
                            ...
                            EM_IR | EM_WRONG_EXP,
                                                        ER_Error, "Only variables references allowed, expressions not allowed.");
                        }
                        else
                        {
                            ...
                            if (vi == NULL)
                            {
                                Error::getInstance ()->add (e->lineNum (), EM_IR | EM_VARIABLE_NOT_FOUND, ER_Error, "%s", sname.c_str ());
                            }
                            if (!vi->isState ())
                            {
                                Error::getInstance ()->add (e->lineNum (), EM_IR | EM_WRONG_EXP, ER_Error, "Only state variables allowed.");
                            }
                            else
                            {
                                _lhs->insert (_getIndex (e->getAsComponentReference (), vi), DEP_STATE);
                    ...
                    case BIF_TERMINATE:
                    {
                        ...
                        {
                            Error::getInstance ()->add (eca->lineNum (), EM_IR | EM_WRONG_EXP, ER_Error, "Expected 1 arguments, found %d",
                                                        eca->arguments ()->size ());
                        }
                    }
                        break;
...
MMO_Statement_::_insertDeps (AST_Statement stm, vector<int> begin, vector<int> end)
{
    ...
    if (begin != end)
    {
        ...
        _insertVectorDeps (deps, _deps, DEP_DISCRETE, DEP_DISCRETE_VECTOR, begin, end);
        _insertVectorDeps (deps, _deps, DEP_ALGEBRAIC, DEP_ALGEBRAIC_VECTOR, begin, end);
        deps = ls->lhs ();
        _insertVectorDeps (deps, _lhs, DEP_STATE, DEP_STATE_VECTOR, begin, end);
        _insertVectorDeps (deps, _lhs, DEP_DISCRETE, DEP_DISCRETE_VECTOR, begin, end);
        _insertVectorDeps (deps, _lhs, DEP_ALGEBRAIC, DEP_ALGEBRAIC_VECTOR, begin, end);
                                  ...
                                  int offset, int order, int forOffset, list<string>& ret)
{
    ...
    list<string> code = eq->print (indent, index, idx, true, _data->algebraics (), EQ_HANDLER, order, false, offset, false, forOffset);
    vi->setEachModifier (mod);
    stringstream reverse;
    if (vi->isParameter () && (vi->type ()->getType () == TYINTEGER))
    {
        list<Index>::iterator it = idxs.begin ();
        reverse << "__reverse__PAR_" << vi->name () << "[" << index << "-1] = " << it->print (idx, offset) << "+1;";
    }
...
void
MMO_Statement_::_printList (AST_StatementListIterator it, AST_StatementList stl, const string& indent, const string& idx, int offset, int order,
...
void
MMO_Statement_::_printIfExpression (AST_Expression e, string lhs, const string& indent, const string& idx, int order, int offset, int forOffset,
                                    list<string>& ret, list<string>& code)
{
    ...
    _data->setCalculateAlgegraics (false);
    _data->setLHS (tmp);
    exp->print (indent, lhs, idx, true, _data->algebraics (), EQ_HANDLER_IF, order, false, offset, false, forOffset);
...
list<string>
MMO_Statement_::print (string indent, string idx, int offset, int order, int forOffset)
{
    list<string> ret;
    stringstream buffer;
    switch (_stm->statementType ())
    {
        case STIF:
        {
            ...
            _printIfExpression (e, "if", indent, idx, order, offset, forOffset, stms, code);
            stms.push_back ("{");
            AST_StatementListIterator it;
            AST_StatementList stl = _stm->getAsIf ()->statements ();
            ...
            foreach(eilit,eil)
            {
                _printIfExpression (current_element(eilit)->condition (), "else if", indent, idx, order, offset, forOffset, stms, code);
                stms.push_back ("{");
                stl = current_element(eilit)->statements ();
                _printList (it, stl, indent, idx, offset, order, forOffset, stms);
                stms.push_back ("}");
        ...
        case STASSING:
        {
                ...
                switch (Util::getInstance ()->checkBuiltInFunctions (name))
                {
                    case BIF_REINIT:
                    {
                        ...
                        foreach(eli,eca->arguments())
                        {
                            ...
                            {
                                _printAssignment (cRef->name (), cRef, e, indent, idx, offset, order, forOffset, ret);
                            }
                            c++;
                        }
                        buffer.str ("");
            ...
            {
                _printAssignment (name, cr, sa->exp (), indent, idx, offset, order, forOffset, ret);
            }
        }
            break;
        case STFOR:
