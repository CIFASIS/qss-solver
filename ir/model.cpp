...
{
  _types = TypeSymbolTable();
  _declarations = VarSymbolTable();
  _declarations->initialize(_types);
  _derivatives = MMO_EquationTable();
  _algebraics = MMO_EquationTable();
  _outputs = MMO_EquationTable();
  _events = MMO_EventTable();
...
void
MMO_Model::setExternalFunctions(MMO_FunctionTable ft)
{
...
string
MMO_Model::printInitialAssignment(VarInfo vi, string indent, string localVar)
{
...
int
MMO_Model::_evalExp(AST_Expression exp)
{
...
list<string>
MMO_Model::_getFunctionDependencies(map<string, string> mdeps)
{
  list<string> ret;
...
Index
MMO_Model::_getAlgebraicIndex(AST_Expression left, Range range)
{
...
void
MMO_Model::_setAlgebraicOffset(AST_Expression left, Range range)
{
  AST_Expression_ComponentReference cr = left->getAsComponentReference();
  string varName = _getComponentName(cr);
  VarInfo vi = _declarations->lookup(varName);
...
void
MMO_Model::_setAlgebraic(AST_Expression left, AST_Expression right,
...
void
MMO_Model::_insertEquation(AST_Equation eq, Range range)
{
  if(range.check())
  {
...
void
MMO_Model::_insertAlgebraicEquation(AST_Equation eq, Range range)
{
  if(range.check())
  {
    Error::getInstance()->add(eq->lineNum(), EM_IR | EM_UNKNOWN_ODE, ER_Error,
...
void
MMO_Model::_insertFunctionDependencies(list<string> deps,
...
int
MMO_Model::outputIndexes(Index idx)
{
  return _outputIndexes[idx];
}

AST_Statement_ElseList
MMO_Model::_transformStatementElse(AST_Statement_ElseList stel)
{
...
AST_StatementList
MMO_Model::_transformStatementList(AST_StatementList sts)
{
...
AST_Statement
MMO_Model::_transformStatement(AST_Statement st)
{
...
string
MMO_Model::_transformExpression(AST_Expression l, AST_Expression r)
{
...
bool
MMO_Model::_controlScalarExpression(AST_Expression exp)
{
...
VarInfo
MMO_Model::_variableLookup(string name, AST_Expression exp, ER_Type type)
{
...
MMO_Model::_getComponentReference(AST_Expression exp)
{
...
double
MMO_Model::_scalarValue(AST_Expression exp)
{
...
void
MMO_Model::_insertAlgebraic(AST_Equation eq, Range range)
{
  if(eq->equationType() == EQEQUALITY)
  {
    ExpressionIndex_ ei(_declarations);
...
void
MMO_Model::_equationTraverse(void
    (MMO_Model::*tr)(AST_Equation, Range range))
{
...
void
MMO_Model::setEquations()
{
  _equationTraverse(&MMO_Model::_insertAlgebraic);
  _algebraicEquations = 1;
  _equationTraverse(&MMO_Model::_insertAlgebraicEquation);
  _equationTraverse(&MMO_Model::_insertEquation);
}

MMO_SymbolRefTable
MMO_Model::calledFunctions()
{
...
bool
MMO_Model::hasExternalFunctions()
{
  return _calledFunctions->count() > 0;
}

void
MMO_Model::_controlDiscreteVariables(AST_Expression expEq)
{
...
void
MMO_Model::_controlEquation(AST_Equation eq)
{
  if(eq->equationType() == EQEQUALITY)
  {
    AST_Equation_Equality eqe = eq->getAsEquality();
...
void
MMO_Model::_setIndex(AST_Expression derArg, VarInfo vi, Index *idx)
{
  if(_isArray(derArg))
  {
    idx->setLow(1);
...
void
MMO_Model::_setRealVariables(AST_Equation eq, Range range)
{
  if(range.check())
  {
    Error::getInstance()->add(eq->lineNum(), EM_IR | EM_UNKNOWN_ODE, ER_Error,
...
AST_Equation
MMO_Model::_transformEquation(AST_Equation eq)
{
  if(eq->equationType() == EQEQUALITY)
  {
    MMO_ReplaceInnerProduct rip(_declarations);
...
void
MMO_Model::_insertEvent(AST_Statement stm, Range range)
{
  if(stm->statementType() == STWHEN)
  {
    Index idx(0, range);
...
void
MMO_Model::_getFunctionInfo(MMO_Function f)
{
...
MMO_EquationTable
MMO_Model::algebraics()
{
  return _algebraics;
}

MMO_EquationTable
MMO_Model::derivatives()
{
  return _derivatives;
}
...
MMO_Function::MMO_Function_(string name) :
    _name(name), 
    _externalFuncs(0), 
    _outputs(0), 
    _outputName(), 
...
void
MMO_Function::setImports(MMO_ImportTable it)
{
...
MMO_ArgumentsTable
MMO_Function::externalFunctionCalls()
{
...
list<string>
MMO_Function::localDeclarations()
{
  list<string> ret;
  stringstream buffer;
...
void
MMO_Function::setPrefix(string prefix)
{
  _prefix = prefix;
}

string
MMO_Function::prototype()
{
  stringstream input;
  stringstream output;
  stringstream func;
...
MMO_Package::MMO_Package_(string name) :
    _name(name), 
    _funcs(0), 
    _packages()
{
...
bool
MMO_Model::_isArray(AST_Expression exp)
{
  if(exp->expressionType() == EXPCOMPREF)
  {
    AST_Expression_ComponentReference ecr = exp->getAsComponentReference();
...
void
MMO_Function::setFunctions(MMO_FunctionTable functions,
