        ...
        _annotations (), _derivative (), _include ()
{
    ...
    _annotations.insert (pair<string, MMO_FunctionAnnotation_::type> ("derivative", DERIVATIVE));
    _annotations.insert (pair<string, MMO_FunctionAnnotation_::type> ("Include", INCLUDE));
    _annotations.insert (pair<string, MMO_FunctionAnnotation_::type> ("IncludeDirectory", INCLUDE_DIRECTORY));
    _annotations.insert (pair<string, MMO_FunctionAnnotation_::type> ("Library", LIBRARY));
    _annotations.insert (pair<string, MMO_FunctionAnnotation_::type> ("LibraryDirectory", LIBRARY_DIRECTORY));
}
...
{
    string annot = *(x->name ());
    map<string, MMO_FunctionAnnotation_::type>::iterator itf = _annotations.find (annot);
...
MMO_ModelAnnotation_::_processArgument (AST_Argument_Modification arg)
{
    if (arg->hasModification ())
    {
        ...
        {
            Error::getInstance ()->add (m->lineNum (), EM_IR | EM_ANNOTATION_TYPE, ER_Error, "%s", arg->name ()->c_str ());
        }
    }
    else
    {
        Error::getInstance ()->add (arg->lineNum (), EM_IR | EM_ANNOTATION_TYPE, ER_Error, "%s", arg->name ()->c_str ());
    }
}
...
{
    string annot = *(x->name ());
    map<string, MMO_ModelAnnotation_::type>::iterator itf = _annotations.find (annot);
    if (itf == _annotations.end ())
    {
        return (false);
    }
...
void
MMO_ModelAnnotation_::_processExpressionList (AST_Expression x, list<AST_Expression> *l)
{
    if (x->expressionType () == EXPBRACE)
    {
        AST_Expression_Brace b = x->getAsBrace ();
...
MMO_AnnotationValue
MMO_EvalAnnotation_::foldTraverseElement (MMO_AnnotationValue e1, MMO_AnnotationValue e2, BinOpType bot)
{
    MMO_AnnotationValue av = MMO_AnnotationValue ();
    switch (bot)
    {
