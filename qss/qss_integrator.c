...
QSS_SEQ_printSimulationLog (QSS_simulator simulator)
{
    ...
    SD_print (simulator->simulationLog, "Total time: %g ms", simulator->stats->initTime + simulator->stats->simulationTime + simulator->stats->saveTime);
#ifdef DEBUG
...
QSS_SEQ_logMemory (QSS_simulator simulator)
{
    ...
    int xOrder = data->order + 1, states = data->states, events = data->events, sDouble = sizeof(double), sInt = sizeof(int), discretes = data->discretes,
...
{
    unsigned long totalSimSteps = 0, totalLPSteps = 0, maxSteps = simulator->stats->steps[0], minSteps = maxSteps, totalExternalEvents = 0, totalMessages = 0;
    ...
    SD_print (simulator->simulationLog, "Processed messages: %.2f %%", (totalMessages > 0) ? ((double) totalExternalEvents / (double) totalMessages) * 100 : 0);
    ...
    SD_print (simulator->simulationLog, "Additional memory allocated: %.2lf MBytes (%.2f %%)", memoryIncrement * sm, (memoryIncrement / (double) lps) * 100);
#ifdef DEBUG
    QSS_generateWeights (simulator);
#endif
    SD_print (simulator->simulationLog, "");
...
void
QSS_PAR_sendSimultaneousMessages (QSS_simulator simulator, QSS_externaEventHandler externalEvent, QSS_internalEventHandler internalEvent)
{
