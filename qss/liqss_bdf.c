...
#endif
{
  ...
  quantizer->state->a =
      (double *)malloc(infs * sizeof(double)); // vector jacobiano
  quantizer->state->u0 =
      (double *)malloc(infs * sizeof(double)); // vector restos orden cero
  quantizer->state->u1 =
      (double *)malloc(infs * sizeof(double)); // vector restos orden uno
  // Inicializaciones
  for (i = 0; i < infs; i++) {
  ...
  for (i = 0; i < states; i++) {
    for (m = 0; m < simData->nSD[i]; m++) {
      ...
      quantizer->state->a[quantizer->state->qss_bdf->S[simData->SD[i][m]][i]] =
          simData->jac[s++];
    }
  }
...
void LIQSS_BDF_solver2x2_h(QA_quantizer quantizer, double *x, double *q,
  ...
  double M[4] = {1, 0, 1, -1}, H[4] = {1, 0, 0, 0},
         *invM = M; // modify H[4] = hnew/hold
  double hmin = quantizer->state->qss_bdf->hmin;
  double RTE;

  // invAd = I-h*A
  double **invAd = quantizer->state->qss_bdf->invAd;
  if (quantizer->state->qss_bdf->band & 1) // pasos con LIDF2
  {
    ...
    for (i = 0; i < nBE; i++) {
      for (j = 0; j < nBE; j++) {
        if (S[BES[i]][BES[j]] != -1) {
          if (i == j)
            invAd[i][j] = 1 - (2 / 3.0) * h * a[S[BES[i]][BES[j]]];
          else
            invAd[i][j] = -(2 / 3.0) * h * a[S[BES[i]][BES[j]]];
        } else {
          if (i == j)
            invAd[i][j] = 1;
          else
  ...
  } else // primer paso con LIE
  {
    for (i = 0; i < nBE; i++) {
      for (j = 0; j < nBE; j++) {
        if (S[BES[i]][BES[j]] != -1) {
          if (i == j)
            invAd[i][j] = 1 - h * a[S[BES[i]][BES[j]]];
          else
            invAd[i][j] = -h * a[S[BES[i]][BES[j]]];
        } else {
          if (i == j)
            invAd[i][j] = 1;
          else
  ...
  dgetri_(&nBE, invAd[0], &nBE, pivotArray, lapackWorkspace, &nn,
          &errorHandler);
  ...
  } else // LIDF2
  {
    ...
    for (i = 0; i < nBE; i++) {
      ...
      for (j = 0; j < nBE; j++) {
        if (S[BES[i]][BES[j]] != -1) {
          if (i == j)
            aux[i] += ((4 / 3.0) - (2 / 3.0) * h * a[S[BES[i]][BES[j]]]) *
                      q[3 * BES[j]];
          else
            aux[i] += (-(2 / 3.0) * h * a[S[BES[i]][BES[j]]]) * q[3 * BES[j]];
        } else {
          if (i == j)
            aux[i] += (4 / 3.0) * q[3 * BES[j]];
        }
      }
    }
    // inversa * paréntesis
    ...
    if (RTE < 1 && 1) {
      ...
      if (quantizer->state->qss_bdf->smaller >
          4) // controlo que el error sea chico en varios pasos
      {
        h = h * RTE * 0.8; // recalculo h con margen de 80% de RTE
        // printf("- %g\n", RTE);
        if (h < hmin)
          h = hmin; // Control de que no se achique demasiado el paso => evita
                    // cuelgues
        // printf("%g  -\n", h/quantizer->state->h);
        ...
        for (i = 0; i < nBE; i++)
          xprev[i] =
              (invM[2] * H[0] * M[0] + invM[3] * H[3] * M[2]) * q[3 * BES[i]] +
              (invM[2] * H[0] * M[1] + invM[3] * H[3] * M[3]) * xprev[i];
      }
    }
    // Error muy chico => bigger el paso
    else if (RTE > 1 && 1) {
      quantizer->state->qss_bdf->bigger++;
      if (quantizer->state->qss_bdf->bigger >
          4) // controlo que el error sea chico en varios pasos
      {
        // if(RTE>2) RTE = 2;
        h = h * RTE * 0.8; // recalculo h con margen de 80% de RTE
        ...
        // h *= 2;
        if (h < hmin)
          h = hmin; // Control de que no se agrande demasiado el paso
        ...
        {
                printf("\nF\n");
                quantizer->state->h = quantizer->state->finTime*(1-1e-9) -
        quantizer->state->lSimTime->time;
                for(i=0; i<nBE; i++)
                                xprev[i] =
        (invM[2]*H[0]*M[0]+invM[3]*H[3]*M[2])*q[3*BES[i]] +
        (invM[2]*H[0]*M[1]+invM[3]*H[3]*M[3])*xprev[i];
    ...
    // printf("%g\n", quantizer->state->finTime -
    // (quantizer->state->lSimTime->time + h));
    // Hago que termine justo en el t final
    if (quantizer->state->lSimTime->time + h > quantizer->state->finTime && 1) {
      // printf("\nF\n");
      quantizer->state->qss_bdf->h = quantizer->state->finTime * (1 - 1e-9) -
                                     quantizer->state->lSimTime->time;
  ...
  if (!BE[var]) {
    ...
    if (flag2[var] != 1) {
      if ((quantizer->state->lSimTime->type == ST_Event) && (a[var] < 0) &&
          (quantizer->state->nSZ[var] > 0)) { // we check if var is involved in
                                              // the zero crossing function that
                                              // produced the current event
      ...
      // if (fabs(aold-a[var])>fabs(a[var])/2) nTime[var] = t +
      // quantizer->state->finTime * quantizer->state->minStep*rand()/RAND_MAX;;
      //      nTime[var] = t + quantizer->state->finTime *
      //      quantizer->state->minStep*0;
      if (self) {
        flag2[var] = 2;
        nTime[var] = t;
      }
...
#endif
{
  ...
  if (BE[var]) {
    ...
    if (change[var] == TRUE)
      ...
      if (var == BES[0]) {
        ...
        for (j = 0; j < nBE; j++)
          change[BES[j]] = TRUE;
#ifdef QSS_PARALLEL
        LIQSS_BDF_PAR_solver2x2_h(quantizer, x, q, lqu);
#else
    ...
    if (a[S[var][var]] < 0) {
      if (ddx == 0) {
        ddx = a[S[var][var]] * a[S[var][var]] * q[cf0] +
              a[S[var][var]] * u0[S[var][var]] + u1[S[var][var]];
        if (ddx == 0)
          ddx = 1e-40;
      }

      h = (quantizer->state->finTime - t);
      q[cf0] = ((x[cf0] + h * u0[S[var][var]] + h * h / 2 * u1[S[var][var]]) *
                    (1 - h * a[S[var][var]]) +
                (h * h / 2 * a[S[var][var]] - h) *
                    (u0[S[var][var]] + h * u1[S[var][var]])) /
               (1 - h * a[S[var][var]] +
                h * h * a[S[var][var]] * a[S[var][var]] / 2);
      ...
      q[cf1] =
          (a[S[var][var]] * q[cf0] + u0[S[var][var]] + h * u1[S[var][var]]) /
          (1 - h * a[S[var][var]]);
