...
PRT_setMetisOptions (idx_t options[METIS_NOPTIONS], QSS_data data)
{
    ...
    for (i = 0; i < t; i++)
        {
            options[PRT_getMetisOption (data->params->partitionerOptions.metis[i].lvalue)] = PRT_getMetisOption (
...
void
PRT_generateIntStructure (int *n, int *e, int *np, int **x, int **xa, int **vw, int **ew, int **val, grp_t nvtxs, grp_t edges, grp_t nparts, grp_t *xadj,
