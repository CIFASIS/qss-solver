...
QString FileItem::plotStr()
{
  ...
  if (plot) {
    ...
    if (lines) ret.append("with lines");
    if (steps) ret.append("with steps");
...
MapFiles::~MapFiles() { delete _files; }

QList<FileItem> MapFiles::files(QString file) { return _files->value(file); }

void MapFiles::clear() { _files->clear(); }

void MapFiles::addFiles(QString dir, bool init)
{
  QDir d(dir);
...
void MapFiles::clearFiles(QString dir) { _changeFiles(dir, false); }

void MapFiles::setFiles(QString dir) { _changeFiles(dir, true); }

FileItem MapFiles::fileItem(QString dir, QString name)
{
...
void MapFiles::setFileItem(FileItem it, QString dir, QString name)
{
  /*	QList<FileItem> fitems = _files->value(dir);
...
void MapFiles::_changeFiles(QString dir, bool st)
{
  QList<FileItem> fitems = _files->value(dir);
