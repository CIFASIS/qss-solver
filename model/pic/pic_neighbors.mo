model pic_neighbors
    ...
    parameter Integer phi_idx = 3*(N_PART + N_CELLS);
    ...
    discrete Real qx, qy, qz;
    discrete Real px, py, pz;
    discrete Real rho_neigh1[N_PART], rho_neigh2[N_PART], rho_neigh3[N_PART], rho_neigh4[N_PART];
    ...
    initial algorithm
        ...
        for i in 1:N_CELLS loop
            ...
            rho := spwt * (particlesInsideVolume(i) / getVolumeCapacity(i));
            rho := setVolumeProperty(i, "rho", rho);
        ...
        for i in 1:N_PART loop
            when time > nextCrossingTimeOf(i,x[i],y[i],z[i],vx[i],vy[i],vz[i]) then
                ...
                if neighbors == 5 then
                    ...
                    rho := setVolumeProperty(volumeID[i], "rho", rho - spwt * (rq/cap));
                ...
                if volumeID[i] == 0 then
                    ...
                    if neighbors == 5 then
                        qx := rayIntersectionXOf(volumeID[i],x[i],y[i],z[i],vx[i],vy[i],vz[i]);
                        qy := rayIntersectionYOf(volumeID[i],x[i],y[i],z[i],vx[i],vy[i],vz[i]);
                        qz := rayIntersectionZOf(volumeID[i],x[i],y[i],z[i],vx[i],vy[i],vz[i]);
                        ...
                        neighborID1 := getNeighborID(volumeID[i], 1);
                        d1 := distanceToVolume(neighborID1, px, py, pz);
                        d := d + d1;

                        neighborID2 := getNeighborID(volumeID[i], 2);
                        d2 := distanceToVolume(neighborID2, px, py, pz);
                        d := d + d2;

                        neighborID3 := getNeighborID(volumeID[i], 3);
                        d3 := distanceToVolume(neighborID3, px, py, pz);
                        d := d + d3;

                        neighborID4 := getNeighborID(volumeID[i], 4);
                        d4 := distanceToVolume(neighborID4, px, py, pz);
                        d := d + d4;
