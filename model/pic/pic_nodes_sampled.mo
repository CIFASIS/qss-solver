model pic_nodes_sampled
    ...
    initial algorithm
        ...
        for i in 1:N_PART loop
            ...
            if i <= np_insert then
                evt_time[i] := 0;
                next_charge_time[i] := dt;
                removeFrom[i] := particle_currentVolumeID(i);
            else
                vx[i] := 0;
                vy[i] := 0;
                vz[i] := 0;
    ...
    algorithm
        ...
        for i in 1:N_PART loop
            ...
            when time >= next_charge_time[i] then
                rho := volume_getVertexProperty(removeFrom[i], 5, "rho") - rho_vertex1[i];
                rho := volume_setVertexProperty(removeFrom[i], 5, "rho", max2(rho,0));

                rho := volume_getVertexProperty(removeFrom[i], 6, "rho") - rho_vertex2[i];
                rho := volume_setVertexProperty(removeFrom[i], 6, "rho", max2(rho,0));

                rho := volume_getVertexProperty(removeFrom[i], 7, "rho") - rho_vertex3[i];
                rho := volume_setVertexProperty(removeFrom[i], 7, "rho", max2(rho,0));

                rho := volume_getVertexProperty(removeFrom[i], 8, "rho") - rho_vertex4[i];
                rho := volume_setVertexProperty(removeFrom[i], 8, "rho", max2(rho,0));
                ...
                if volumeID[i] > 0 then
                    ...
                    next_charge_time[i] := time + dt;
                end if;

                removeFrom[i] := volumeID[i];
            end when;
            ...
            when time > particle_nextCrossingTime(i,x[i],y[i],z[i],vx[i],vy[i],vz[i]) then
                volumeID[i] := particle_currentVolumeID(i);
                removeFrom[i] := particle_previousVolumeID(i);

                if volumeID[i] == 0 then
                    reinit(vx[i], 0);
                    reinit(vy[i], 0);
                    reinit(vz[i], 0);
                    next_charge_time[i] := time;
                end if;
            end when;
