...
MOD_definition (int i, double *x, double *d, double *alg, double t, double *dx)
{
    ...
    switch (i)
    {
        ...
        default:
            ...
            {
                alg[(j) * 3] = x[(j + 2) * 3];
                alg[(j + 2) * 3] = (log (d[(j + 16)] * (36 * d[(j + 10)] - alg[(j) * 3]) / (36 * d[(12)]) + 1.0) * __PAR_K * __PAR_Ac * d[(13)]
                        / __PAR_q - alg[(j) * 3] * __PAR_Rspv / 36) * 36;
                alg[(j + 4) * 3] = alg[(j + 2) * 3];
                alg[(j + 6) * 3] = ((d[(j + 4)]) * x[(j + 2) * 3] - x[(j) * 3]) / ((d[(j + 2)]) + (d[(j + 4)]));
...
MOD_dependencies (int i, double *x, double *d, double *alg, double t, double *der, int *map)
{
    ...
    {
        alg[(j) * 3] = x[(j + 2) * 3];
        alg[(j + 2) * 3] = (log (d[(j + 16)] * (36 * d[(j + 10)] - alg[(j) * 3]) / (36 * d[(12)]) + 1.0) * __PAR_K * __PAR_Ac * d[(13)] / __PAR_q
                - alg[(j) * 3] * __PAR_Rspv / 36) * 36;
        alg[(j + 4) * 3] = alg[(j + 2) * 3];
        alg[(j + 6) * 3] = ((d[(j + 4)]) * x[(j + 2) * 3] - x[(j) * 3]) / ((d[(j + 2)]) + (d[(j + 4)]));
        for (j4 = 0; j4 <= 1; j4++)
        {
...
QSS_initializeDataStructs (QSS_simulator simulator)
{
    ...
    modelData->d[(12)] = __PAR_Ior * pow ((modelData->d[(13)] / __PAR_Tref), 3.0)
            * exp (__PAR_q * __PAR_Eg * (1.0 / __PAR_Tref - 1.0 / modelData->d[(13)]) / __PAR_K / __PAR_Ac);
    for (i10 = 0; i10 <= 1; i10++)
    {
