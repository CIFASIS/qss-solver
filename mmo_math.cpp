...
MMOMath::MMOMath(bool replace,
    map<string, pair<list<string>, ASTNode*> > *functions, string prefix) :
    _algebraics(), _currentAlgebraic(0), _type(MATH_EQUATION), _functions(
    NULL), _replace(replace), _imports(), _currentImport(0), _asgVariable(), _isConditional(
        false)
{
  if(_replace)
  {
...
MMOMath::_replaceFunctions(ASTNode* node)
{
  if(node->isFunction())
  {
    map<string, pair<list<string>, ASTNode*> >::iterator it = _functions->find(
        node->getName());
    if(it != _functions->end())
    {
      ...
      pair<string, list<string> > algDef = _generateAlgebraic(it->second, args,
          node);
      _algebraics.insert(
