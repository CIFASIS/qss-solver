...
MmomeGui::_run (QString name)
{
    ...
    env.insert ("MMOC_PACKAGES", QDir (_utils->appDir (MMOC_PACKAGES)).absolutePath ()); // Add an environment variable
...
for (int i = 0; i < MaxRecentFiles; ++i)
{
    ...
    connect (recentFileActs[i], SIGNAL (triggered ()), this, SLOT (_openRecentFiles ()));
}
